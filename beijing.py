...
class BJMyFrame(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id=1, title = '北京PM2.5查询', pos = (150,100), size = (500,600))
        panel = wx.Panel(self)  #创建画板
#创建标题，并设置字体
        self.title = wx.StaticText(panel,label = '北京PM2.5查询', pos = (200,50))
...
#再判断输入时间是否符合
        if year and month and day and hour:
            ...
            draw_trend_chart(data,'北京',str(year)+"年"+str(month)+"月"+str(day) +"号")

        elif year and month and not day and not hour:
            self.COut.SetValue("")
            data = mysql3(year, month)
            draw_trend_chart(data,'北京',str(year)+"年"+str(month) + "月（每日平均值）")
#点击清空按钮
