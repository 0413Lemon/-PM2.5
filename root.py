...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id=1, title = 'PM2.5查询', pos = (150,100), size = (500,600))
...
#再判断输入时间是否符合
        if site and year and month and day and hour:
            if site == '北京':
                ...
                self.COut.SetValue("")
                #db.rollback()
                self.COut.AppendText('地点错误')
            # except:
            #     message='错误'
            #     db.rollback()
            #     self.COut.AppendText(message)
                #弹出提示框
    #断开
            # cursor.close()
            # db.close()
